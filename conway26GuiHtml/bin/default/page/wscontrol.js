...
function sendCmdToServer(cmd) {//funzione triggerata da evento click
...
function  initWS(){
	...
	socketToGui.onopen = () => {
 	...
 	/*3*/
	socketToGui.onmessage = (event) => {//qua riceviamo il messaggio dal server
     	console.log("initWS | onmessage:",event.data);
		
		//qua c'è la parte di decoding del messaggio ricevuto da server
	 	if( event.data.startsWith("ID:")){//qua messaggio misterioso
			console.log("initWS | onmessage:",event.data);
			pageId= event.data.split(":")[1];
			addItem( "page ID="  + pageId ); 
		}
		
		else if( event.data.startsWith("cell(")){ //deve ricevere da caller, questo messaggio che ci dice di cambiare valore della cella
			//addItem(event.data);
...
initWS()//quindi inizializziamo il web server e creiamo la griglia aggiungendo gli eventi
